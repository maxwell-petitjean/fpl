...
Original file is located at
    https://colab.research.google.com/drive/197QmJy2je6xriRW9i0BcmNkKS4ncCcSi
"""
...
def run_model(
    ...
    budget,
    picks_data,
    optimisation_mode="next_6_gw",  # NEW
):
    ...
    # ------------------------------------------------------------------------------------------
    # ---- TEAMS data ----
    teams = pd.DataFrame(json1['teams'])[['id','name','short_name',
                                          'strength_attack_home','strength_defence_home',
                                          'strength_attack_away','strength_defence_away']]
    teams.columns = ['team_id','team_name','team_code',
                     'str_o_h','str_d_h','str_o_a','str_d_a']
    ...
    players1 = players[['id','first_name','second_name','team','element_type','now_cost',
                        'selected_by_percent','clearances_blocks_interceptions','recoveries',
                        'tackles','clean_sheets','expected_assists','expected_goals',
                        'total_points','minutes']]
    players2 = (
        players1
        .merge(teams[['team_id', 'team_code']], left_on='team', right_on='team_id')
    ...
    players3['cbrit'] = (players3['clearances_blocks_interceptions'] +
                         players3['recoveries'] +
                         players3['tackles'])
    players4 = players3[['id_x','name','team_code','pos','now_cost','selected_by_percent',
                         'cbrit','clean_sheets','expected_assists','expected_goals',
                         'total_points','minutes']]
    players4.columns = ['id','name','team','pos','cost','ownership','dc','cs',
                        'xa','xg','points','mins']

    # ------------------------------------------------------------------------------------------
    # ---- FORM -- LAST 10 (THIS YEAR)
    form0 = pd.json_normalize(gw_df['elements'])
    form1 = form0[['id','stats.defensive_contribution','stats.clean_sheets',
                   'stats.expected_assists','stats.expected_goals',
                   'stats.total_points','stats.minutes']]
    ...
    fixtures_prev3 = fixtures_prev2[['name','cbrit','clean_sheets',
                                     'expected_assists','expected_goals',
                                     'total_points','minutes']]
    fixtures_prev3.columns = ['name_form','dc_form','cs_form','xa_form',
                              'xg_form','points_form','mins_form']
    ...
    form6.columns = ['name_form','pos_form','cs_form','dc_form','mins_form',
                     'points_form','xa_form','xg_form']
    ...
    form10['xgp_form'] = np.where(
        (form10['pos_form'] == 'FWD'), form10['xg_form'] * 4,
        np.where(
            (form10['pos_form'] == 'MID'), form10['xg_form'] * 5,
            np.where((form10['pos_form'] == 'DEF') | (form10['pos_form'] == 'GKP'),
                     form10['xg_form'] * 6, 0)
        )
    )
    ...
    form12['dcp_form'] = np.where(
        (form12['pos_form'] == 'MID'), form12['dc_form'] / 6,
    ...
    form12['pred_pp90_form'] = (form12['xopp90_form']+
                                form12['cspp90_form']+
                                form12['dcpp90_form']+2)
    ...
    players5 = (
        players4
        .merge(players_prev8, how="left", left_on='name', right_on='name_ly')
        .merge(form13,    how="left", left_on='name', right_on='name_form')
    )

    # ------------------------------------------------------------------------------------------
    # ---- Build fixture difficulty ----
    team_gw_prev1 = fixtures_prev1[['GW','position','team','opponent_team',
                                    'was_home','total_points','minutes']]
    ...
    team_gw_prev2['pos'] = np.where(
        (team_gw_prev2['position'] == 'MID')|(team_gw_prev2['position'] == 'FWD'),
        'ATT',
    ...
    team_gw_prev8['team_new'] = np.where(
        team_gw_prev8['team_code'] == VAR_REL1, VAR_PRO1,
        np.where(
            team_gw_prev8['team_code'] == VAR_REL2, VAR_PRO2,
            np.where(team_gw_prev8['team_code'] == VAR_REL3, VAR_PRO3, team_gw_prev8['team_code'])
        )
    )
    team_gw_prev9 = team_gw_prev8.merge(teams[['team_code','team_id']],
                                        left_on='team_new', right_on='team_code')
    ...
    lookup_team1 = dict(zip(teams[['team_id','team_code']].team_id,
                            teams[['team_id','team_code']].team_code))
    ...
    for df in [fixtures_home_att,fixtures_home_def,fixtures_home_gk,
               fixtures_away_att,fixtures_away_def,fixtures_away_gk]:
        df.columns = new_cols
    ...
    players8['xm_max'] = np.where(players8['flag_fpl'].isna(),
                                  players8['xm_form'],
                                  players8['xm_form'] * (players8['flag_fpl']/100))
    ...
    players9['mean_value'] = round(
        players9[['pred_pp90_form','pp90_ly','ep_fpl']].mean(axis=1),
        2
    )
    players9['mean_value'] = np.where(
        players9['pp90_ly']==0,
        players9[['pred_pp90_form','ep_fpl']].mean(axis=1),
        players9['mean_value']
    )
    ...
    player_output = players10[['name','team','pos','ownership','points','mins',
                               'xm','points_ly','xg','xa','cs','dc',
                               'predicted_points','base_points','mean_value',
                               'xopp90_form','pred_pp90_form','pp90_ly','ep_fpl']]
    ...
    player_output = player_output[['name','team','pos','ownership','points','mins','points_ly',
                                   'xg','xg_p90','xa','xa_p90','cs','dc','dc_p90',
                                   'xopp90_form','pred_pp90_form','base_points','predicted_points']]
    player_output = player_output.fillna(0)
    player_output = player_output.sort_values(by='predicted_points', ascending=False)

    output = players10[['name','team','pos','pos_id','cost','ownership',
                        'predicted_points','xm','fdr',
                        'gw1','gw2','gw3','gw4','gw5','gw6']]
    output.columns = ['name','team','pos','pos_id','cost','ownership',
                      'predicted_points','xm','fdr',
    ...
    if optimisation_mode == "free_hit":
        weeks = [VGW_NAME_1]  # Free Hit – this GW only
    else:
    ...
    for team in output['team'].unique():
        prob += pulp.lpSum(player_vars[i] for i in output.index
                           if output.loc[i, 'team'] == team) <= 3
    ...
    selected_team['starting_weeks'] = selected_team.index.map(
        lambda i: ', '.join([w for w in weeks if week_vars[w][i].value() == 1])
    )
    ...
    selected_team = selected_team[
        ['name','team','pos','cost','ownership','predicted_points',
         'xm','fdr',
         VGW_NAME_1,VGW_NAME_2,VGW_NAME_3,VGW_NAME_4,VGW_NAME_5,VGW_NAME_6,
         'starting_weeks']
    ]

    # 👇 This is your stable interface
