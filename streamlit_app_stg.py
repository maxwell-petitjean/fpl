# -*- coding: utf-8 -*-
"""Copy of streamlit_app_gw12_1.py

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vkq6MCLu_CkMyOsSuM-NY1917vh3Dimc
"""
...
st.caption("Get Ready for Gameweek "+str(VAR_GW)+" (gw"+str(VAR_GW)+")! Hit 'Run Model' to get started.")
...
# ================== INPUT FORM LOGIC ==================
def get_inputs():
    fpl_id = st.text_input("FPL ID (enter your id to pull through your team data, or remove to see ideal wildcard team)")
...
if fpl_id_input:
    try:
        picks_url = f"https://fantasy.premierleague.com/api/entry/{fpl_id_input}/event/{VAR_GW0}/picks/"
        st.session_state.picks_data = requests.get(picks_url).json().get("picks", [])
    except Exception:
...
def run_model(fpl_id, transfers, exclude_names, exclude_teams, include_names, budget, picks_data):
    ...
    fh = output[['name','team','pos','pos_id','cost','ownership','xm','fdr','gw1']]
    fh1 = fh.sort_values(by='gw1', ascending=False)
    ...
    return selected_team, output, player_output, fixtures_att1, fixtures_def1, current_names, fh1
...
if st.session_state.final_team is not None and st.session_state.raw_output is not None:
    ...
    # --- Tab 1 — Final Squad
    with tab1:
        # 👉 Transfer Impact (current team vs selected team)
        st.subheader("🔁 Transfer Impact")

        current_names_set = set(st.session_state.current_names or [])
        ...
        # lookup from pool for removed, from selected for added
        ...
        removed_df = pool.loc[pool["name"].isin(removed_names), ["name","team","pos","predicted_points"]] \
                        .sort_values("predicted_points", ascending=False)
        added_df = sel_df.loc[sel_df["name"].isin(added_names)] \
                        .sort_values("predicted_points", ascending=False)

        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"**Out ({len(removed_df)}):**")
            if removed_df.empty and fpl_id_input:
                st.info("No players removed.")
            elif not fpl_id_input:
                st.caption("Enter an FPL ID to compare against your current team.")
            else:
                st.dataframe(removed_df.round(2), use_container_width=True, height=220)

        with c2:
            st.markdown(f"**In ({len(added_df)}):**")
            if added_df.empty and fpl_id_input:
