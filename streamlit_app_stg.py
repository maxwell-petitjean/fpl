...
def run_model(fpl_id, transfers, exclude_names, exclude_teams, include_names, budget, picks_data=None):
    ...
    if fpl_id and picks_data is None:
        ...
        picks_data = requests.get(picks_url).json().get("picks", [])
    ...
    return selected_team, output, player_output, fixtures_att1, fixtures_def1, picks_data
...
if "picks_data" not in st.session_state:
    st.session_state.picks_data = None
...
if st.button("🚀 Run Model"):
    ...
    with st.spinner("Optimising your squad... please wait ⏳"):
        final_team, output, player_output, fdr_att, fdr_def, picks_data = run_model(
...
if st.session_state.final_team is not None and st.session_state.raw_output is not None:
    ...
    with tab1:
        ...
        # Helper to fetch current names (full name = first + second)
        def _fetch_current_names(fpl_id_val):
            if not fpl_id_val:
                return []
            try:
                ids = [p["element"] for p in st.session_state.picks_data]
                players_data = requests.get(URL1).json()["elements"]
                id_to_full = {p["id"]: f"{p['first_name']} {p['second_name']}" for p in players_data}
                return [id_to_full[i] for i in ids if i in id_to_full]
            except Exception:
                return []

        current_names = set(_fetch_current_names(fpl_id_input))
        ...
        removed_names = sorted(current_names - selected_names)
        added_names   = sorted(selected_names - current_names)

        # lookup predicted_points for removed players in the raw candidate pool
        ...
        # (Existing squad table styling, unchanged)
